    ...
    return(
      bestofferdata.length===0?<><Shimmer></Shimmer></>:
        <>
          <div className="sticky top-0 left-0 z-50"> 
            {offerimageseting===true? 
               <div className="absolute top-0 bg-red-500 text-red-950 w-10 h-12 text-5xl text-center ml-[150px]">
                  <span onClick={()=>setOfferImageSeating(false)}  class="material-symbols-outlined">
                   close
                   </span>
               </div>:
               <div className="absolute top-0 bg-green-600 text-red-950 w-10 h-12 text-5xl  text-center ml-[150px]">
                <span onClick={()=>setOfferImageSeating(true)} class="material-symbols-outlined">
                open_in_full
                </span >
          ...
          {
            offerimageseting===true?  <img className="" src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_900/`+bestofferdata?.data?.cards[0]?.card?.card?.imageId}></img>:null
          }       
            
          </div>
          <div className="flex flex-wrap items-center justify-center mt-6">
            {
             bestofferdata?.data?.cards?.map((data,index)=>{return index>1?<Card data={data?.card?.card}></Card>:null})
            }
          </div>
        </>
    )
