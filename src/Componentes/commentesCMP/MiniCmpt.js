...
console.log("rep",data.replies);
  return (
    <>
       
         <form
         onSubmit={(e) => e.preventDefault()}
         className="mb-3 p-2 border rounded bg-white shadow-md transform hover:scale-105 transition-transform duration-300 perspective-500 lg:w-3/4"
       >
         <div className="flex items-center mb-1">
           <p className="font-bold text-blue-500 text-sm mr-1">{data.name}</p>
         </div>
         <p className="text-red-500 text-sm">{part.to}{<span className="text-gray-500 text-sm">{part.message}</span>}</p>
         ...
         {/* like and dislike syboles */}
           <span role="img" aria-label="thumbs up" className="mr-1">
             <img className="w-10 h-10" src="https://media.istockphoto.com/id/1136351242/vector/like-social-media-hand-line-icon-editable-stroke-pixel-perfect-for-mobile-and-web.jpg?s=612x612&w=0&k=20&c=OnKkq5JCHkLvZ1Ck_njtTQMCyLljXsXGNhGqVpwwVUA="></img>
           </span>
 
           <span role="img" aria-label="thumbs down" className="mr-1">
           <img className="w-10 h-5" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSLObItZTGqju1OqgykJ21mYHcvIGR7cIlLXZAbMKe0ug&s"></img>
           </span>
           <p
         ...
         >
           <span className="mr-2">
             {data.replies.length}{showReplayes ? " Hide Replies" : " Show Replies"}
           </span>
           <svg
         ...
         <div className="ml-4 mt-2">
             {showReplayes && <ComponentFun data={data.replies} replay={indexKey} replayAva={true} />}
         </div>
       </form>
